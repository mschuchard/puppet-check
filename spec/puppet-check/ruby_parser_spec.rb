...
describe RubyParser do
  ...
  context '.ruby' do
    ...
    it 'puts a bad style ruby file in the clean files hash when rubocop_args ignores its warnings' do
      ...
      expect(PuppetCheck.settings[:error_files]).to eql([])
      expect(PuppetCheck.settings[:warning_files]).to eql([])
      expect(PuppetCheck.settings[:clean_files]).to eql({ "#{fixtures_dir}lib/rubocop_style.rb" => nil })
    end
    it 'puts a good ruby file in the clean files hash' do
      RubyParser.ruby(["#{fixtures_dir}lib/good.rb"], true, [])
      expect(PuppetCheck.settings[:error_files]).to eql([])
      expect(PuppetCheck.settings[:warning_files]).to eql([])
      expect(PuppetCheck.settings[:clean_files]).to eql({ "#{fixtures_dir}lib/good.rb" => nil })
    end
  end
  ...
  context '.template' do
    ...
    it 'puts a ruby template file with ignored errors in the clean files hash' do
      RubyParser.template(["#{fixtures_dir}templates/no_method_error.erb"])
      expect(PuppetCheck.settings[:error_files]).to eql([])
      expect(PuppetCheck.settings[:warning_files]).to eql([])
      expect(PuppetCheck.settings[:clean_files]).to eql({ "#{fixtures_dir}templates/no_method_error.erb" => nil })
    end
    it 'puts a good ruby template file in the clean files hash' do
      RubyParser.template(["#{fixtures_dir}templates/good.erb"])
      expect(PuppetCheck.settings[:error_files]).to eql([])
      expect(PuppetCheck.settings[:warning_files]).to eql([])
      expect(PuppetCheck.settings[:clean_files]).to eql({ "#{fixtures_dir}templates/good.erb" => nil })
    end
  end
  ...
  context '.librarian' do
    ...
    it 'puts a bad style librarian Puppet file in the clean files hash when rubocop_args ignores its warnings' do
      RubyParser.librarian(["#{fixtures_dir}librarian_style/Puppetfile"], true, ['--except', 'Layout/ArgumentAlignment,Style/HashSyntax'])
      expect(PuppetCheck.settings[:error_files]).to eql([])
      expect(PuppetCheck.settings[:warning_files]).to eql([])
      expect(PuppetCheck.settings[:clean_files]).to eql({ "#{fixtures_dir}librarian_style/Puppetfile" => nil })
    end
    it 'puts a good librarian Puppet file in the clean files hash' do
      RubyParser.librarian(["#{fixtures_dir}librarian_good/Puppetfile"], true, [])
      expect(PuppetCheck.settings[:error_files]).to eql([])
      expect(PuppetCheck.settings[:warning_files]).to eql([])
      expect(PuppetCheck.settings[:clean_files]).to eql({ "#{fixtures_dir}librarian_good/Puppetfile" => nil })
    end
  end
end
