...
describe PuppetParser do
  ...
  context '.manifest' do
    ...
    it 'throws a well specified error for an invalid PuppetLint argument' do
      ...
      expect { PuppetParser.manifest([fixtures_dir + 'manifests/style_lint.pp'], false, true) }.to raise_error(RuntimeError, 'puppet-lint: invalid option supplied among --non-existent --does-not-exist')
    end
  end
